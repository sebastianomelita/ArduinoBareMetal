>[Torna a reti di sensori](sensornetworkshort.md#servizi-di-accesso-radio-per-WSN)>[Torna a multiplazione TDM](mezzoradio.md#Multiplazione-TDM)
...
**Ascolto del canale** (detto anche ascolto della portante) può essere considerato il **processo** di valutare **continuativamente** l'energia che in esso è presente, ad esempio, mediante misure di **potenza**. Lo scopo è stabilire quando la **potenza osservata** è significativamente maggiore di quella che normalmente è presente su un canale **a riposo** (idle). Ciò può essere rilevato mediante un dispositivo a **soglia** che scatti oltre un certo valore di **riferimento**. Si tratta di una rilevazione **diretta** di **canale occupato**.
...
In altre parole, se **due stazioni** A e B trasmettono una di seguito all'altra separate di un tempo Δt e il tempo di propagazione tra A a B ΔtAB è maggiore di Δt allora, ascoltando la portante:
- B sente il canale libero e inizia a trasmettere il suo messaggio mentre c’è quello di A che sta per arrivare.
- ciascuna stazione riterrà di essere l’unica a trasmettere sul canale perchè, al tempo Δt, il canale appare ancora libero. Dopo il tempo di propagazione ΔtAB (corrispondente alla distanza tra A e B) la trama di A, che ha trasmesso per prima, arriverrà all’altra stazione B, che aveva cominciato dopo ritenendo il canale libero, collidendo con la trasmissione già in corso.
...
6. Dopo un numero massimo di volte in cui, **ascoltando durante** la trasmissione, si continua a **rilevare** collisioni, allora la stazione abbandona l’idea di trasmettere (linea interrotta o guasta).
...
6. Dopo un numero massimo di volte in cui, **ascoltando durante** la trasmissione, si **rilevano** ancora collisioni, allora la stazione abbandona l’idea di trasmettere (linea interrotta o guasta).
...
Nel grafico, DATA e ACK rappresentano dei **tempi di trasmissione** (non di propagazione), cioè il tempo che impiega il TX a trasmettere l'intero messaggio.
...
- viene **decrementato** periodicamente dall'orologio di bordo di ciascuna stazione, anche quando questa **non deve trasmettere**
...
Riassumendo, esistono due tipi di CCA (Clear Channel Assesment):
...
Il messaggio **CTS** è ricevuto da **tutte le stazioni**: 
     - quelle che **non hanno prenotato** il canale (con un RTS), col tempo in esso riportato, **impostano** il proprio contatore di Carrier Sensing detto **NAV** con il quale tengono conto della prenotazione. Il NAV viene **decrementato** periodicamente nel tempo, fino a 0; quando il NAV è **diverso da zero**, vuol dire che una trasmissione è in atto nelle vicinanze e quindi, anche dovendolo fare, la stazione, per il momento, si astiene dal trasmettere.
     - l'unica stazione che **ha prenotato** il canale (con il corrispondente RTS), sa che dal momento della ricezione del CTS può **cominciare** a trasmettere e che può continuare a farlo in sicurezza per tutta la **durata del NAV**. Poichè il tempo di trasmissione NAV è al riparo dalle collisioni, di fatto, questo meccanismo rende **impossibili** gli eventi di collisione per i **messaggi dati**.
