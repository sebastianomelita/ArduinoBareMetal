...
- **Altrimenti** la stazione **arresta** la trasmissione corrente e **trasmette** invece una particolare sequenza di **32 byte**, detta **sequenza di jamming**, che avvisa della collisione sia chi non trasmette ancora e che, per chi sta già trasmettendo, abbia una durata tale da rendere possibile che la collisione possa essere rilevata in **ogni situazione** anche dalle stazioni più lontane. Infatti, nel caso in cui la collisione, avvenga tra stazioni **molto vicine**, la collisione potrebbe essere rilevata (e quindi interrotta) troppo presto per essere rilevata ovunque nel BUS dato che due onde sovrapposte brevi e viaggianti in senso opposto, dopo un certo tempo, si separano (vedi Fig.7).
