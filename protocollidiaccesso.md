...
- Altrimenti la stazione **arresta** la trasmissione corrente e **trasmette** invece una particolare sequenza di **32 byte**, detta **sequenza di jamming**, che avvisa della collisione chi non trasmette ancora e che, per chi sta già tramettendo, abbia una durata tale che possa essere rilevata dalle stazioni più lontane anche nel caso in cui la collisione, avvenendo tra stazioni **troppo vicine**, risulti interrotta troppo presto per esserlo (due onde sovrapposte brevi dopo un certo tempo si separano).
