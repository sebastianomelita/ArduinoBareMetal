...
- l'ingresso sul sensore di stop determina due stati ma in momenti diversi, uno prima del blocco del codice con ```waitUntilInputLow``` (```PEZZO_PRONTO```) e uno dopo (```TRASPORTO_STIMATO```) , ne consegue che anche in questo caso lo switch-case interno è superfluo.
