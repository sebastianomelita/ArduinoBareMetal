...
- **Per semplificare il codice**. Può essere fatto usando la parola chiave **yield**, usando **async/await** per **"appiattire"**, cioè ridurre ad un **unico livello di annidamento** il codice asincrono evitando l'inferno delle callback o scrivendo codice asincrono in **stile imperativo** cioè lineare e sequenziale, molto più naturale da scrivere e mantenere rispetto ad un groviglio di callback correlate tra loro.
