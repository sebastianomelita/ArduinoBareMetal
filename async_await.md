...
- le funzioni dichiarate **asincrone** con async davanti **restituiscono** sempre una **future** per cui await forza il completamento in serie delle operazioni asincrone quando il risultato dell'operazione successiva dipende dal risultato dell'ultima. In altre parole, una funzione normale che chiama una funzione asincrona (coroutine) **ritorna sempre** dopo essere stata eseguita **per intero** mentre, la **coroutine** ritornerà successivamente, **a tappe**, man mano che gli eventi che essa, di volta in volta, attende verrano, uno dopo l'altro, **risolti tutti**.
