...
- in **Python**, il meccanismo è del tutto analogo ma, in questo caso, il loop dei messaggi non è nativamente gestito dal linguanggio come in JS, ma deve essere attivato esplicitamente, ad esempio, con ```asyncio.run(main())```. Inoltre il main non è di base un task gestito dal loop dei messaggi ma dalla schedulatore dei thread del SO sottostante.
