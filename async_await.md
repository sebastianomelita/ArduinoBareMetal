...
- In **Javascript**, ad esempio, una funzione **normale sincrona** che chiama una funzione asincrona (coroutine) **ritorna sempre** dopo essere stata eseguita **per intero**, mentre la **coroutine** ritornerà successivamente alla funzione chiamante (non asincrona), seguendo un percorso **a tappe**, man mano che gli eventi che essa, di volta in volta, attende verranno, **risolti tutti**. Rimane la particolarità del JS di trattare il main che, pur essendo una funzione sincrona, viene considerata alla stregua di un task come un altro, anzi è proprio il **primo task** che viene inserito nella coda dei messaggi.
