...
- **Per semplificare il codice**. Può essere fatto usando la parola chiave **yield**, usando **async/await** per **"appiattire"**, cioè ridurre ad **un unico livello di annidamento** il codice asincrono evitando l'inferno delle callback o scrivendo codice asincrono in **stile imperativo** cioè lineare esequenziale.

- **Uso efficiente delle risorse** del sistema operativo e dell'hardware. In presenza di apllicazioni con molti thread, si puoò trarre vantaggio dalle coroutine risparmiando sull'allocazione della memoria, il tempo necessario per eseguire il cambio di contesto e, in definitiva, trarre vantaggio dall'utilizzo più efficiente dell'hardware che, a parità di risorse, adesso può eseguire più task. Un esempio è l'utilizzo dell'IO non bloccante con molti utenti simultanei. Poiché in generale i thread sono più costosi dei socket, esaurirai i thread del sistema operativo disponibili più velocemente dei socket. Per evitare questo problema è possibile utilizzare l'IO non bloccante con le coroutine.
...
- in **Python**, il meccanismo è del tutto analogo ma, in questo caso, il loop dei messaggi non è nativamente gestito dal linguanggio come in JS, ma deve essere attivato esplicitamente, ad esempio, con ```asyncio.run(main())```. Inoltre il main non è un task gestito dal loop dei messaggi ma dalla schedulatore dei thread del SO sottostante.
