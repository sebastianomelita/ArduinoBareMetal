...
- le funzioni dichiarate **asincrone** con async davanti **restituiscono** sempre una **future** per cui await forza il completamento in serie delle operazioni asincrone quando il risultato dell'operazione successiva dipende dal risultato della precedente. In altre parole, una funzione **normale** che chiama una funzione asincrona (coroutine) **ritorna sempre** dopo essere stata eseguita **per intero** mentre, la **coroutine** ritornerà successivamente alla funzione chiamante (non asincrona), seguendo un percorso **a tappe**, man mano che gli eventi che essa, di volta in volta, attende verranno, uno dopo l'altro, **risolti tutti**.
