...
- **creando un'unica rete** utilizzando livelli di rete **compatibili** o **praticamente uguali** come sono **IPV6 e 6LowPan**. Lo stesso deve accadere per i **protocolli di routing** usando ad es. REPL. Lo stesso deve accadere per i **livelli superiori**. In particolare nell'ultimo, il **livello applicativo**, il **payload dell'applicazione**, cioè il messaggio utente, deve poter viaggiare **direttamente** dal **client** al **sensore**, dove poi verrà elaborato e utilizzato. Se viene inoltrato direttamente il pacchetto IPV6 (con il payload applicativo dentro) allora il gateway è tecnicamente un **router semplice**.
    ...
    - i protocolli di livello fisico e data link (L1 e L2 OSI) sono generalmente diversi mentre quelli di livello di rete e trasporto (L3 e L4 della pila OSI) sono esattamente gli stessi. Per effetto di ciò, ad esempio, una rete WiFi può comunicare direttamente con una rete Zigbee (o Thread) mediante gateaway router a cavallo tra le due.
    - la **traduzione del livello applicativo** (L7 pila OSI), quella che definisce il tipo si protocollo Client/Server End to End e il tipo di semantica applicativa (cioè il formato dei messaggi per accendere una tapparella piuttosto che una lampada), non deve necessariamente avvenire sul gateway ma può essere svolta direttamente sul server applicativo, dato che il payload potrebbe non essere mai sbustato fino alla destinazione. Potrebbe persino non essere affatto necessaria se tutti i dispositivi condividono lo stesso protocollo applicativo (ad esempio Matter).
...
- utilizzando un **gateway applicativo** per **compattare** o **tradurre** i messaggi:
     ...
     - la **traduzione del livello applicativo** (L7 pila OSI), quella che definisce il tipo si protocollo Client/Server End to End e il tipo di semantica applicativa, normalmente è svolta sul gateway di confine tra la rete di sensori e la rete di distribuzione, dato che comunque il payload deve essere sbustato.
