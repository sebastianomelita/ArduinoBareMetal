...
- utilizzando un **gateway applicativo** per **compattare** o **tradurre** i messaggi:
     ...
     - la **traduzione del livello applicativo** (L7 pila OSI), quella che definisce il tipo di **semantica applicativa** (il formato JSON per le tapparelle piuttosto che quello per le lampade), normalmente è svolta sul **gateway di confine** tra la rete di sensori e la rete di distribuzione, dato che comunque il payload in un gateway deve comunque essere sbustato.
