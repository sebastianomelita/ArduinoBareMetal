...
- **lungo raggio**. Sono parecchie e di varie epoche, al momento le più diffuse sono:
   ...
   - Per scopi di **allerta precoce** nella prevenzione dei disastri potrebbero essere prese in considerazione anche tecnologie come LoRaWan. Il **wifi mesh** può essere adoperato per la realizzazione di **connettività estemporanea** in caso di eventi o calamità naturali. In **scenari emergenziali** o post disastro il modello **mesh peer-to-peer** di **Meshtastic (LoRa mesh)** è più rilevante del modello stellare di LoRaWan che dipende da gateway ed è spesso tenuto attivo in collaborazione con le realtà radioamatoriali del territorio.
