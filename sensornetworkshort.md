...
Dal sensore al network server, uno stesso messaggio percorre generalmente **due tratte** di tipo diverso utilizzando un differente **formato del payload**. I messaggi scambiati in una rete wireless sono complessivamente di due tipi che si mappano l’uno sull’altro:
...
- **nodo sensore di rete**. Dispositivo che in un unico **contenitore** contiene **insieme** un certo numero di **sensori**, la **MCU**, la **scheda di rete** verso la **rete di accesso** ai sensori, **batteria** di alimentazione. E' un nodo unico con tutto quello che serve per misurare e comunicare nella rete di sensori.
- **nodo MCU con scheda di rete**. In questo caso il nodo è costituito da una MCU con delle interfacce digitali verso i sensori e una interfaccia di rete verso la rete dei sensori. Il dispositivo è quindi un **gateway** tra la **il BUS di campo** (porte analogiche/digitali, BUS I2C, BUS RS485, RS232, ecc.) e la rete di sensori (WiFi, Zigbee, LoraWAN, LAN, BLE, ecc.). Esistono due possibilità:
    ...
    -  L'**interfaccia sui sensori** usa un collegamento **digitale a BUS** verso un **gruppo** di sensori, per cui l'**architettura** risultante dei loro collegamenti è a **BUS**, avente la MCU come **master** del BUS.
