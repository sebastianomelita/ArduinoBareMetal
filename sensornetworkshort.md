...
- utilizzando un **gateway** per **tradurre** o **compattare** i messaggi:
     - se il routing non è compatibile ma il livello di applicazione nella rete di accesso **è customizzabile**, si può pensare ancora di utilizzare lo stesso payload della rete di distribuzione in formato JSON richiamando le primitive di servizio di livello applicativo in uso nella rete dei sensori. Ma il formato potrebbe essere troppo prolisso per dispositivi di capacità ridotta.
