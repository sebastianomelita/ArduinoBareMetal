...
- **mqttReconnectTimer** è sempre attivo **se è presente il WiFi** e serve a fare il **polling** dello **stato** della connessione **MQTT** tramite **mqttClient.connected()**. Se MQTT **è connesso** (mqttClient.connected() restituisce true ) non accade nulla altrimenti viene lanciato un tentativo di riconnessione tramite connectToMqtt().
