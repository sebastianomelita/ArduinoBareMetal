...
- Il **server OpenVPN** può inviare ai client le configurazioni DNS utilizzando la direttiva push "dhcp-option". Il **client OpenVPN** può modificarla localmente tramite la **direttiva** ```dhcp-option DNS 192.168.10.1``` nel **file di configurazione** OpenVPN del client (in questo esempio forza il dns locale).
