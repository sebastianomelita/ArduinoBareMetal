...
l'HCI (Hyper-Converged Infrastructure - infrastruttura iperconvergente) è un framework IT software-defined che porta lo storage **dentro** ogni nodo di compute. I dischi di tutti i nodi vengono visti come un unico pool distribuito, accessibile da qualsiasi VM del cluster. Ogni scrittura viene confermata su almeno N nodi fisici diversi prima di essere completata (replica sincrona).
