...
Il client adesso invia al server la chiave di sessione appena creata su un canale sicuro cifrandola con la chiave pubblica del server (non gode di PFS). Oppure sia il client che il server utilizzano lo stesso algoritmo di Diffie Helmann per ottenere indipendentemente due chiavi OTP uguali (gode di PFS).
