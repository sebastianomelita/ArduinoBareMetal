...
Fornisce un canale multicast di livello applicativo (L7) tramite cui un utente col ruolo di publisher è in grado di notificare una replica dello stesso messaggio a più subscribers **identificati** mediante un topic, assimilabile ad un **indirizzo di destinazione** di gruppo L7. Se si volesse identificare un solo dispositivo allora sono possibili due alternative:
- un topic che lo individua univocamente, magari all'interno di un topic che individua gerarchicamente il suo gruppo:
- un topic che identifica il suo gruppo di appartenenza su cui si individua un certo dispositivo mediante una proprietà univoca contenuta nel messaggio JSON come un ID_dispositivo: 
