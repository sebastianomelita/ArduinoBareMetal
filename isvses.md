...
I **link** sulle **porte** di uno **SW**, pur essendo anch'essi dei **BUS**, raramente collegano **molti host** con uno stesso filo, piuttosto sono, similmente a degli hub, degli **aggregatori** di collegamenti **punto-punto** verso **singoli host**. Oltre a essere colegamenti **punto-punto**, i link verso gli host sono anche **full duplex**, motivo per cui su di essi risulta impossibile pure la residua possibilità di collisione tra i dati di SW e Host nel caso in cui entrambi parlino nello stesso istante (inviano dati su canali simplex separati per le due direzioni).
