...
I dispositivi di **livello 2** sono un **barriera** per le singole PDU di livello 1, cioè i bit, che, invece di essere subito inoltrate, vengono accomodate su una **coda di ingresso** fino a che non viene completato il caricamento di un intera PDU di livello 2, cioè una **trama**. Inoltre, sono dispositivi che non propagano le collisioni eventualmente verificatesi nei link multpunto sulle loro porte. Uno switch, infatti, è anche un limite invalicabile per la propagazione delle **trame danneggiate** provenienti dai dispositivi di livello 1, in quanto le trame, memorizzate sulla coda di ingresso, vengono analizzate controllandone dimensione e **checksum FCS** al fine di stabilirne la correttezza. 
