...
-  un'**astrazione logica** del timer HW che si possa **instanziare molte volte** associandola ad una **propria callback**. I timer HW sottostanti rimangono sempre solamente 4.
...
La possibilità di poter instanziare un **timer logico** per **ogni task** definito all'interno di una certa **callback** rende la **libreria Ticker** (e la sottostante implentazione ```esp_timer```) una strada **effettivamente percorribile** per la realizzazione di uno **schedulatore di compiti** sia periodici che one shot (esecuzione singola non ripetuta). La **schedulazione** dei compiti rimane **molto precisa** perchè collegata a ISR eseguite da segnali di interrupt lanciati da **timer fisici**.
...
Una **soluzione** potrebbe essere inserire all'interno della **callback** di un timer HW uno schedulatore di compiti basato sui **time tick** con cui poter realizzare il **filtraggio** degli **eventi** da eseguire **nel futuro** come quelli visti nella sezione [Polled timers](indextimers.md) o librerie di **terze parti** come [SimpleTimer](https://github.com/marcelloromani/Arduino-SimpleTimer/tree/master/SimpleTimer) (https://github.com/marcelloromani/Arduino-SimpleTimer/tree/master/SimpleTimer).
