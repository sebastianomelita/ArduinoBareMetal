...
Avere più **flussi di esecuzione paralleli** fornisce quindi il **vantaggio** di poter realizzare gli algoritmi in **maniera lineare** suddividendoli in **fasi sequenziali** la cui **tempistica** può essere stabilità in **maniera semplice** ed intuitiva impostando dei **ritardi**, cioè dei delay(), che **bloccano** temporaneamente l'esecuzione del loro thread tra una fase e l'altra. La **separazione** dei flussi permette una **progettazione indipendente** degli algoritmi eccetto che per i **dati comuni** a più flussi (thread), per i quali deve essere **sincronizzato l'accesso** con opportuni meccanismi. 
