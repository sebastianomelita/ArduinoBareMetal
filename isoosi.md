...
|---------|-----------------------|-----------------------------|-----------------------------|
| 7       | Applicazione          |  Applicazione Client/Server che virtualizza una risorsa locale remota condivisa facendola apparire come locale e dedicata, definisce il **tipo di utenti** e il **tipo di sicurezza** con cui dialogano| HTTP, DNS, DHCP |
| 6       | Presentazione         |  Traduce **documenti**, **flussi di dati**, **record**, **caratteri** da un formato all'altro (ad es. XML piuttosto che JSON oppure mp3 al posto di mp4), realizza la **compressione** dei dati e la loro **cifratura**. Realizza la **serializazione** degli oggetti (ad esempio struct)| MIME, XDR, HTML, XML, SSL, TLS, zip |
