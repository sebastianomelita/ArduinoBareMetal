...
- i **nodi IS** sono i nodi di transito **interni** alla rete, quelli che forniscono il servizio di consegna dei messaggi utilizzato dagli ES per comunicare tra di loro. Svolgono funzioni di smistamrnto di pacchetti per selezionare il percorso ottimale tra nodo ES mittente e nodo ES destinatario, tra i tanti possibili all'interno di una rete parzialmente magliata. I nodi IS sono per ISO/OSI solamente i **router**. Switch e HUB (cavo o wireless) sono assimilabili anch'essi a dispositivi di smistamento ma, per ISO/OSI, non esistono, sono servizi di rete nascosti all'interno di un link L2 o L1. I nodi IS devono implementare obbligatoriamente solo i primi 3 livelli della pila ISO/OSI (fisico, linea e rete).**
