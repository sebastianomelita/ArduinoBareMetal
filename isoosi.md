...
| 3       | Rete                  | **Indirizzamento**, cioè individuazione del luogo fisico in cui si trova l'host destinatario di un pacchetto, esegue la **scelta** del percorso migliore per raggiungerlo (**routing**), esecuzione (in ciascun router) dell'**inoltro fisico** (forwarding) che consiste nello **smistamento** di ciascun pacchetto da una porta di ingresso a quella porta di uscita che sta nel percorso scelto per raggiungere la destinazione di quel pacchetto.  |   IP    |                                                                  |
