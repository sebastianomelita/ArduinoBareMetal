...
| 3       | Rete                  | **Indirizzamento**, cioè individuazione del luogo fisico in cui si trova l'host destinatario, esegue la **scelta** del percorso migliore per raggiungerlo (**routing**), esecuzione dell'**inoltro fisico** (forwarding) dei pacchetti, da una porta di ingresso a quella di uscita che sta nel percorso scelto, per ogni router incontrato lungo il percorso.  |   IP    |                                                                  |
...
Un **canale virtuale** è un **canale logico** che **emula** un **collegamento diretto** tra entità pari di uno **stesso livello**, in cui le **PDU** di questo **sembra** che siano scambiate proprio con le regole del **protocollo** che sono state definite per quel canale virtuale. I **canali virtuali** vengono **creati** grazie alla tecnica dell'**imbustamento multiplo** e rappresentano il **punto di vista** che un **generico livello** ha dell'unico canale effettivamente esistente, cioè il canale reale di livello 1 (fisico). 
