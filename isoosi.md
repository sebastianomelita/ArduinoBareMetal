...
Una **eccezione** notevole fanno le **LAN** cioè le reti locali composta da switch. Una **intera rete** LAN composta da molti switch collegati insieme con dei link di livello 1, viene considerata come un unico link (collegamento) di livello 2 **diretto tra due router**. Ciò accade per una anomalia introdotta storicamente dall'impiego delle LAN consistente nell'utilizzo combinato di router e switch. Questa usanza ha, di fatto, introdotto due **livelli di routing** complementari, di cui solo quello esterno **tra i router** è, in realtà, preso in considerazione dell'architettura ISO/OSI. Riassumendo abbiamo due livelli di smistamento:
