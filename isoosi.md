...
Una **eccezione** notevole fanno le **LAN**, dove una **intera rete** LAN composta da molti switch collegati insieme con dei link, viene considerata come un unico link (collegamento) di livello 2 **diretto tra due router**. Ciò accade per una anomalia dovuta storicamente dall'uso delle LAN.  L'utilizzo combinato di router e switch ha introdotto due **livelli di routing** complementari, di cui solo quello esterno **tra i router** è, in realtà, preso in considerazione dell'architettura ISO/OSI:
