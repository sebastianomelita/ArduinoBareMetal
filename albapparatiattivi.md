...
- se **non coincide** col **link fisico**, vuol dire che questo è stato sostituito da una connessione logica L2, formata dalla trame MAC della **rete carrier** nazionale, della quale attraversa **molti router**, fino ad un server PPPoE nella centrale del gestore della connessione payload IP, dove la connessione L3 viene terminata su un suo router. Questa **connessione logica** è realizzata tipicamente tramite il protocollo di **tunnelling PPPoE**. QUindi, se nella sede del gestore ci sta il **server PPPoE**, allora nel router utente deve chiaramente essere installato anche un **client PPPoE** per poter realizzare il tunnel.
