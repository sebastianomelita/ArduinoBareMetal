...
- se il **task** genera un **ritardo** nel ```loop()``` **maggiore** di un tbase, allora il tempo del primo tick successivo verrà calcolato subito dopo il ritardo e questo tempo in genere **non è** un multiplo intero di ```tbase```. Al tick successivo (e a tutti quelli a seguire) invece si arriva partendo dal polling del tempo corrente fino a cercare il prossimo multiplo intero di ```tbase```. Al **primo tick** dopo il ritardo il tempo attuale potrebbe cumulare **molti multipli** di ```tbase``` per cui più tick successivi potrebbero essere eseguiti **tutti insieme** uno dopo l'altro.
