...
3. se il **task** genera un **ritardo** nel ```loop()``` **maggiore** di un ```tbase```, allora il tempo del primo tick successivo verrà calcolato **subito dopo** il ritardo e questo tempo in genere **non è** un multiplo intero di ```tbase```, per cui potrebbe essere introdotto un **errore**. Al **primo tick** dopo il ritardo, il tempo attuale potrebbe cumulare **molti multipli** di ```tbase``` per cui più **tick consecutivi** potrebbero essere eseguiti **tutti insieme** uno dopo l'altro, fino a che ```current_millis-precm < tbase```. I tick **successivi al primo** dopo il ritardo ricadono nella situazione al punto 1. e sono tutti esattamente **multipli** di ```tbase```.
