...
## Punto 1 — Infrastruttura di rete
...
La rete **Mesh WiFi** è pensata per completare o sostituire del tutto la rete WiFi infrastruttura cablata. Nasce da un **AP centrale** direttamente collegato al dispositivo **gateway di cantiere** (dispositivo rugged che contiene il router firewall e il core switch). Da questo si diramano i **backhaul** che possono creare **maglie solo wireless** di AP oppure completare le maglie create da AP cablati (soluzione scelta per lo schema proposto). L'architettura è flessibile e **resiliente**, in caso di guasto di un AP o di crollo di un link cablato, un AP isolato si mette automaticamente in modalità mesh e cerca un link wirelesss verso gli altri AP.
...
- **client MQTT** per l'inoltro dei dati dei sensori **non IP** attraverso un **canale applicativo** (L7) realizzato dal **broker MQTT** nella **sede centrale**.
- **router multilivello** col ruolo di cerniera tra la **rete di aggregazione** di sensori WSN e la **rete di trasporto IP** interfacciata attraverso schede di rete **ethernet** o **WiFi**.
...
- **(a) Dispositivo personale — BYOD (non aggiunto al dominio).** Il PC portatile personale si autentica via **802.1X/EAP** verso un server **RADIUS** sulla porta *logica* dell'**AP Wi-Fi** in modalità **WPA2/3-Enterprise**. Poiché il PC portatile **non possiede un account macchina di dominio né un certificato personale**, si adotta **EAP-PEAP / EAP-TTLS**:
     ...
     - dentro il tunnel cifrato viene scambiata la password Radius. RADIUS **valida** la coppia username/password su un database LDAP (lo stesso dell'Active Directory dei PC dell'ufficio) e  assegna la **VLAN dinamica** alla scheda di rete del portatile tramite il `Tunnel-Private-Group-Id` che, inviato dal server Radius, dice all'AP a quale VLAN appartiene l'utente che si collega ad un certo SSID. Il gruppo (VLAN) non viene scelto in base all'SSID ma in base allo username dell'utente.

- **(b) Postazione fissa aziendale (PC aggiunto al dominio Windows).** L'utente esegue il **logon di dominio**, autenticato da **LDAP/Kerberos** verso il **Domain Controller** (AD). Sulla porta *fisica* dello **switch** di sede si potrebbe applicare anche lì **802.1X** con autenticazione **macchina + utente** (credenziali/certificato di dominio) ma è una soluzione eccessiva per dei locali riservati ad impiegati e quindi con accesso inibito ad estranei. L'accesso successivo alle **risorse intranet** sfrutta il **SSO Kerberos**. Il livello di fiducia è **più alto** del BYOD perché il PC è *gestito*, *nel dominio* e sottoposto a GPO dell'Active Directory. Tramite il sevizio SSO di kerberos gli impiegati possono accedere a tutte le risorse interne regolate da **GPO** quali:
     - il proprio desktop Microsoft Windows
     - cartelle sul disco riservate
     - stampanti riservate
...
A sinistra il mondo dei certificati X.509, a destra quello della chiave pre-condivisa per i dispositivi vincolati. Le due figure risolvono lo stesso problema da due approcci opposti (dettagli in [autenticazione di un collegamento](/approfondimenti/autenticazione_collegamento_guida.md):
...
- **Difesa dal MITM nello scambio delle chiavi pubbliche.** L'associazione "chiave pubblica ↔ nodo" va resa autentica, altrimenti un intruso (Mallory) si infila in mezzo. Tre modi:
