>[Torna a svolgimento Smart Road](svolgimento_smart_road.md)>[Torna a Dettaglio architettura RFID](/archrfid.md) 
>
...
**Stima della posizione: trilaterazione vincolata più fingerprinting.** La stima della posizione del beacon BLE non si appoggia su una pura trilaterazione RSSI, perché ciò richiederebbe ovunque almeno tre ancore *non collineari*, condizione non soddisfatta lungo i corridoi dove gli AP sono per forza disposti in fila. Si adotta dunque un approccio misto. Negli spazi in cui la collinearità è rotta da scanner fuori asse — laboratorio, aule di testata di corridoio, CED — si applica trilaterazione RSSI classica con stima della distanza dalla formula log-distance calibrata in sopralluogo. Per le aule centrali, dove le ancore disponibili restano sostanzialmente allineate al corridoio, si applica fingerprinting RSSI vincolato alla planimetria: in fase di collaudo si campiona in ciascuna aula la firma vettoriale di RSSI vista da tutti gli AP udibili, anche quelli "lontani" che contribuiscono comunque alla firma, e in esercizio si confronta la firma corrente del PC con il database delle firme aula per aula. Questo schema risolve in modo nativo l'ambiguità nord/sud lungo il corridoio sfruttando le differenze di propagazione introdotte dai muri e dagli arredi reali dell'edificio.
...
Lo schema di posizionamento risponde a quattro vincoli simultanei. Gli **AP Wi-Fi con radio BLE del corridoio** sono installati a circa otto metri uno dall'altro, in modo che ogni punto del corridoio sia udibile da almeno tre AP: forniscono la posizione lungo l'asse del corridoio, ma essendo collineari non basterebbero da soli a determinare il lato — nord o sud — di pertinenza del PC. Per rompere la collinearità si aggiungono quindi **quattro scanner BLE fuori asse** (quadratini scuri nella planimetria), collocati al centro di laboratorio, aula di testata sinistra (Aula 1), aula di testata destra-bassa (Aula 7) e CED. Le aule centrali, che restano interne al "rettangolo" di ancore non collineari, beneficiano comunque della rottura della simmetria grazie alla differenza di propagazione introdotta dai muri reali: si ricorre alla tecnica di *fingerprinting RSSI* descritta alla sezione 3, in cui ogni aula è caratterizzata da una firma vettoriale di RSSI calibrata in fase di sopralluogo e usata in esercizio per il match della posizione corrente. Il **doppio scanner BLE al varco** (uno interno a circa cinque metri dalla soglia, uno esterno subito fuori) realizza la doppia soglia di prossimità e di attraversamento richiesta dal quesito II. Il **lettore UHF al varco di uscita** è installato in posizione *gantry* sulla soglia e funge da rete di sicurezza per i tag passivi. I **cassoni-carica** sono posizionati nei locali di servizio agli estremi del corridoio, alimentati a rete elettrica e collegati alla rete dati via Wi-Fi.
...
---
## 6.1 ACL dettagliate — politiche di default differenziate

Estensione delle «ACL fondamentali» della sezione 6. Le liste sono **estese, con nome, applicate inbound** sull'interfaccia VLAN dello switch L3 di distribuzione (CED). Politica di default **differenziata per zona**: *default-allow* solo nella LAN di utenti interni fidati (personale), *default-deny* su tutto ciò che è utente non fidato, apparato di campo, server o management.
...
|---|---|---|---|---|
| 10 | management | NMS / amministratore | default-deny | `deny ip any any` |
| 20 | server | broker MQTT, app server, DB | default-deny | `deny ip any any` |
| 30 | wifi-didattica | **studenti** (BYOD, PC in prestito) | **default-deny** | `deny ip any any` |
| 40 | wifi-uffici | **personale / docenti** | **default-allow** | `permit ip any any` |
| 50 | campo-cassoni | cassoni-carica (IoT) | default-deny | `deny ip any any` |
...
ip access-list extended ACL_STUDENTI
 ...
 permit tcp 10.10.30.0 0.0.1.255 any eq www
 permit tcp 10.10.30.0 0.0.1.255 any eq 443
 deny   ip  any any                                       ! ← default deny
!
interface Vlan30
...
ip access-list extended ACL_UFFICI
 ...
 deny   ip  10.10.40.0 0.0.0.255 10.10.10.0 0.0.0.255      ! no management
 deny   ip  10.10.40.0 0.0.0.255 10.10.50.0 0.0.1.255      ! no campo (cassoni 50 + gateway 60)
 deny   ip  10.10.40.0 0.0.0.255 10.10.70.0 0.0.0.255      ! no palmari
 deny   ip  10.10.40.0 0.0.0.255 10.10.30.0 0.0.1.255      ! no rete studenti
 deny   tcp 10.10.40.0 0.0.0.255 host 10.10.20.30 eq 5432  ! no DB diretto
 deny   tcp 10.10.40.0 0.0.0.255 host 10.10.20.10 eq 8883  ! no broker diretto
