...
```
                    ┌────────────────────────────────────┐
                    │   LIVELLO NAZIONALE (CN)           │
                    │   - 2 DC active-active             │
                    │   - DB nazionale, API APP, BigData │
                    └──────────────┬─────────────────────┘
                                   │ MPLS / VPN IPsec su Internet
            ┌──────────────────────┼──────────────────────┐
            │                      │                      │
    ┌───────▼──────┐       ┌───────▼──────┐       ┌───────▼──────┐
    │  CdC Reg. A  │       │  CdC Reg. B  │  ...  │  CdC Reg. N  │
    │  (Edge DC)   │       │  (Edge DC)   │       │  (Edge DC)   │
    └───────┬──────┘       └──────────────┘       └──────────────┘
            │ Fibra dedicata o 5G/4G LTE
   ┌────────┼────────┬─────────┬─────────┐
   │        │        │         │         │
 ┌─▼──┐  ┌─▼──┐  ┌──▼─┐  ┌────▼──┐  ┌───▼──────┐
 │SG-1│  │SG-2│  │SG-3│  │SG-N   │  │ Stazioni │
 │    │  │    │  │    │  │       │  │ ricarica │
 └────┘  └────┘  └────┘  └───────┘  └──────────┘
   smart-gate (telecamere, sensori, maxi-schermi)
```
