...
L'operazione è analoga a quella di prelevare dalla **finestra** di un montacarichi (la banda base) dei **libri** (il segnale) per poi disporli in diversi **scomparti** (le bande) di uno **scaffale** di una **libreria** (lo spettro). Si noti che i libri (persino i più grandi) hanno una **numero di pagine** finite, esattamente come i segnali in banda base hanno una **larghezza di banda** (a 3dB) finita.
