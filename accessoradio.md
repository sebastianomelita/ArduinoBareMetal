...
Tutte le tecnologie di accesso al **canale radio** presuppongono la **traslazione del segnale** da trasmettere dalla sua frequenza originale (detta **banda base**), dove viene **generato**, a quella di **trasmissione** (detta banda **traslata**), dove viene **trasmesso**. L'operazione è analoga a quella prelevare dalla **finestra** di un montacarichi (la banda base) dei **libri** (il segnale) per poi disporli in diversi **scomparti** (le bande) di uno **scaffale** di una **libreria** (lo spettro). Si noti che i libri (persino i più grandi) hanno una **numero di pagine** finite, esattamente come i segnali in banda base hanno una **larghezza di banda** (a 3dB) finita.
...
- **Slow Frequency Hopping (SFH)**. In questo caso **uno o più bit** di dati vengono trasmessi **all'interno** di un **salto**. Un vantaggio è che è possibile un rilevamento coerente dei dati. Spesso, i sistemi che utilizzano l'hopping lento impiegano anche una codifica di controllo degli errori (burst) per ripristinare la perdita di (più) bit in un hop.
- **Fast Frequency Hopping (FFH)**. **Un bit** di dati è **suddiviso** in più salti. Nel **salto veloce**, il rilevamento del segnale coerente è difficile e usato raramente. Principalmente viene utilizzata la modulazione FSK o MFSK.
