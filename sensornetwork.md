...
- a **stella gerarchica** cioè ad albero in cui alcuni dispositivi wireless di smistamento fanno da nodo dell'albero oltre che da **centro stella** di un **cluster** di dispositivi. I dispositivi sensori hanno a disposizione un percorso lungo i rami dell'albero per comunicare con la base station (il gateway). Il **sink tree** o albero di instradamento ottimo è unico per tutti i cluster. La topologia ad albero unico avente per radice la base station (gateway) garantisce un percorso ottimo tra **il gatewaye tutti i nodi** ma non garantisce percorsi ottimi **tra i nodi stessi**. 
- a **maglia** anche detti **flat** o **peer to peer**. In questo caso  ogni nodo si calcola **il proprio** albero di instradamento per raggiungere gli altri nodi e, la base station, in particolare. Gli alberi, avendo radice, di volta in volta diversa, diversa sono, in genere, diversi. Il protocollo di instradamento è spesso distribuito e dinamico e rispetto all'altra soluzione garantisce **percorsi ottimali** per **tutti i nodi**, **non solo** per la base station.
