...
- a **stella gerarchica** In questa categoria, molto popolare è il protocollo Low Energy Adaptive Clustering Hierarchy **(LEACH)**. In LEACH i nodi dei sensori sono divisi in diversi **cluster**. In ogni cluster, il nodo con il livello di energia **più alto** è generalmente scelto come la **testa del cluster** o CH. Il **ruolo del CH** viene **ruotato** allo scopo di distribuire il carico energetico tra i sensori.  Posseggono la funzione di **aggregazione** e **compressione** dei dati provenienti da diversi sensori per **ridurre** il numero di **messaggi trasmessi** alla destinazione. Benchè abbia una **grande efficienza energetica** esistono però delle limitazioni: si assume che i nodi abbiano una potenza di trasmissione tale da poter sempre raggiungere la BS, ipotesi che esclude l’utilizzo in reti estese. Una sua variante detta **piconet** è implementata nelle reti di dispositivi **Buetooth** (IEEE 802.15.1). Ritenuto attualmente particolarmente adatto ai requisiti di una WSN è invece **IETF RPL**. Progettato per le reti **6LoWPAN** è un protocollo proattivo di tipo **distance vector** che genera, attraverzo lo scambio di messaggi DV con i nodi vicini, un albero di instradamento ottimo **unico** per tutti i nodi avente per **radice** la BS (gateway). E' il protocollo di routing adottato da molti framework per IoT come **TinyoS** o **RIOT**.
