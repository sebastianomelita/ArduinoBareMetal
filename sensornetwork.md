...
Esistono anche **stack opensource** rilasciati da alleanze di sviluppatori pensati per sostituire il **livello applicativo** dei chip IoT attualmente più diffusi per orientarli verso ambiti di utilizzo più **specializzato**. Un esempio è **OpenThread** che si pone sopra i chip **802.15.4** realizzando una implementazione dello stack **Thread** una alternativa a Zigbee per l'**industria**, oppure **OpenWSN** che utilizza l'implementazione di TSCH IEEE 802.15.4g/e come protocollo di acceso al mezzo protocolli standard IETF come protocolli di livello superiore (6LowPan, REPL e UDP).
