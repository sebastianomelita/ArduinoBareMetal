...
Esistono sistemi centralizzati con un **solo master** e altri **multimaster** in cui il controllo del BUS passa alternativamente da un master all'altro. Più master richiedono comunque un **meccanismo di sincronizzazione** per evitare **sovrapposizioni**.

 <img src="bus.png" alt="alt text" width="600">
 
In genere i BUS sono in due stati **IDLE** (riposo) e **comunicazione**. Lo stato di **IDLE** è quello **predefinito** ed è ottenuto tramite una resistenza di **pullup** colegata sulla linea. Durante lo stato di **comunicazione** il **valore bit 0** è codificato con il livello basso L=0Volt mentre il **valore bit 1** è codificato con lo stesso valore dell'IDLE cioè **H**. L'**inizio della trasmissione** di una trama è in genere **segnalata** da uno livello basso **L iniziale**.
...
In entrambi i casi, si possono immaginare due **tipi di servizio** che corrispondono ad altrettante **maniere** di gestire **l'interazione** tra un sensore e la rete:
- **servizio sincrono**. Viene predisposto **multiplando** il canale FDM con un **TDM statico**. E' adatto per **operazioni periodiche** come il **polling** di un grappolo di sensori.
- **servizio asincrono**. E' adatto a **operazioni una tantum**,cioè di cui non si riesce a sapere **in anticipo** l'occorenza (accadere nel tempo), ad esempio il comando di accensione di una lampadina.  Viene predisposto **multiplando** il canale FDM con un **TDM a contesa** con accesso al mezzo arbitrato dai protocolli **ALOHA** O **CSMA/CA**. In questo caso **riservare** uno slot temporale ad una sorgente che per la maggior parte del tempo non lo utilizzerebbe sarebbe stato certamente **uno spreco**, per cui si utilizza la tecnica a contesa che **non riserva** nulla.
