...
Talvolta i **gateway** verso la rete di densori, nonostante espongano verso la rete di distribuzione una **interfaccia MAC/IP**, sono scatolotti **proprietari chiusi**, cioè, **non modificabili** o **non adattabili** nella API che espongono e **non bypassabili facilmente** con altri **gateway più aperti**. Questo significa che potrebbe essere necessario per loro un **ulteriore livello di traduzione** per convertire il **formato** delle **API** e dei **protocolli** che espongono con quelle in uso nella rete di distribuzione per tutti i sensori. Ad esempio potrebbe essere necessario un **gateway aggiuntivo** per tradurre delle **API COAP** proprietarie in quelle **MQTT** in uso nella rete di distribuzione. Questa situazione, per i **protocolli più diffusi** come Zibgee e Bluetooth, **tende a scomparire** perchè quasi tutti i produttori ormai **adottano** per i servizi uno **standard comune** ed è possibile comandare dispositivi di una marca col gateway di un'altra. 
