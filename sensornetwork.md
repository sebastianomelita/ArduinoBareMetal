...
Molti sistemi (wifi, zigbee, bluetooth BLE, LoRaWan, Sigfox) permettono di impostare **contemporaneamente**, sulla **stessa interfaccia** radio, il **servizio TDMA** sincrono per le sorgenti che eseguono il **polling** di sensori e un servizio **asincrono** con **ALOHA** o **CSMA/CA** per le sorgenti che devono inoltrare il **comando** di un pulsante di accensione di un attuatore. Ciò è ottenuto inviando sul canale la funzionalità **beacon** con le cosidette **superframe**.
