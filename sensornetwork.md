...
- a **stella gerarchica** In questa categoria, molto popolare è il protocollo Low Energy Adaptive Clustering Hierarchy **(LEACH)**. In LEACH i nodi dei sensori sono divisi in diversi **cluster**. In ogni cluster, il nodo con il livello di energia **più alto** è generalmente scelto come la **testa del cluster** o CH. I nodi non-CH comunicano con i CH attraverso il protocollo **TDMA**, in cui, in base alla pianificazione creata dal CH e inviata a tutti i nodi del cluster, viene stabilita una **turnazione degli slot** temporali assegnati a ciascun nodo per **trasmettere o ricevere**. Il **ruolo del CH** viene **ruotato** allo scopo di distribuire il carico energetico tra i sensori (altrimenti si scaricherebbero per primi).  Posseggono la funzione di **aggregazione** e **compressione** dei dati provenienti da diversi sensori per **ridurre** il numero di **messaggi trasmessi** alla destinazione, il che comporta una **migliore efficienza energetica**. Esistono però delle limitazioni: si assume che i nodi abbiano una potenza di trasmissione tale da poter sempre raggiungere il sink, ipotesi che esclude l’utilizzo in reti estese, inoltre la distribuzione
nella rete dei cluster-head non è omogenea e quindi alcuni nodi potrebbero non avere alcun CH a cui agganciarsi. Una sua variante detta **piconet** è implementata nelle reti di dispositivi **Buetooth** (IEEE 802.15.1). IETF RPL è un'alternativa ritenuta più adatta alle WSN. Si tratta di un protocollo proattivo a vettori di distanza inquadrabile nel contesto del routing gerarchico ed è il protocollo di routing adottato normalmente nelle reti 6LoWPAN.
