...
- a **stella gerarchica** cioè ad albero in cui alcuni dispositivi wireless di smistamento fanno da nodo dell'alberp oltre che da **centro stella°° di un **cluster** di dispositivi. I dispositivi sensori hanno a disposizione un percorso lungo i rami dell'albero per comunicare con la base station (il gateway) che è lo stesso per tutti i cluster. Cioè il sink tree o albero di instradamento è unico per tutti i cluster. La topologia ad albero garantisce l'assenza di percorsi multipli (a patto che il sensore si agganci ad un solo cluster).
- a **maglia** anche detti **flat** o **peer to peer**. In questo caso il percorsi multipli esistono ma ogni nodo si calcola **il proprio** albero di instradamento per raggiungere gli altri nodi e, la base station, in particolare. Il protocollo di instradamento è spesso distribuito e dinamico e rispetto all'altra soluzione garntisce percorsi ottimali per tutti i nodi.
