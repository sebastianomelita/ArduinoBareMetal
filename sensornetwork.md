...
- **commutazione** cioè lo **smistamento** dei dati tra le **dorsali** della rete LAN che, collegando uno switch con l'altro, realizza un **percorso** (path) fino ai router di confine della stessa (**gateway**).
...
- **Hop multiplo**: esiste un percorso verso i nodi terminali passando attraverso dei nodi intermedi. L’energia che un **singolo dispositivo** impiega per raggiungere **un nodo remoto** (ad esempio per accenderlo) si limita a quella per raggiungere il **nodo più prossimo**. Su reti mesh (magliate) di **grandi dimensioni** anche questa energia può essere non trascurabile. Su **reti indoor** è invece la soluzione decisamente **più conveniente** dato che spesso **il primo** dispositivo di smistamrnto raggiungibile (router o switch) non è a più di un metro di distanza mentre quello in **posizione baricentrica** può essere anche a 10-50m di distanza.

Le reti di sensori wireless (WSN) ad **hop multiplo** in realtà sono di **due tipi**:
- a **stella gerarchica** cioè ad albero in cui la base station fa da radice mentre alcuni dispositivi wireless **speciali** fanno da **nodo dell'albero** oltre che da **centro stella** di un **cluster** di dispositivi. I dispositivi sensori hanno adesso a disposizione un percorso ottimizzato (più breve o più veloce) lungo i rami dell'albero per comunicare con la base station (il gateway). Il **sink tree** o albero di instradamento **ottimo** è **unico** per tutti i cluster. La topologia ad **albero unico** avente per **radice la base station**  (gateway) garantisce un percorso ottimo tra **il gateway e tutti i nodi** ma, di per se, non garantisce percorsi ottimi **tra i nodi stessi**. 
- a **maglia** anche detti **flat** o **peer to peer**. In questo caso  ogni nodo si calcola **il proprio** albero di instradamento ottimo per raggiungere gli altri nodi e la base station in particolare. Gli alberi, avendo, di volta in volta, per **radice un nodo** sono generalmnte diversi. Il protocollo di instradamento è spesso distribuito e dinamico e, rispetto all'altra soluzione, garantisce **percorsi ottimali** per **tutti i nodi**, **non solo** per la base station.
