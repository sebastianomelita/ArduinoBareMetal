...
Le reti di sensori wireless (WSN) da **hop multiplo** in realtà sono di **due tipi**:
- a **stella gerarchica** cioè ad albero in cui alcuni dispositivi wireless di smistamento fanno da **nodo dell'albero** oltre che da **centro stella** di un **cluster** di dispositivi. I dispositivi sensori hanno adesso a disposizione un percorso ottimizzato (più breve o più veloce) lungo i rami dell'albero per comunicare con la base station (il gateway). Il **sink tree** o albero di instradamento **ottimo** è **unico** per tutti i cluster. La topologia ad albero unico avente per radice la base station (gateway) garantisce un percorso ottimo tra **il gatewaye e tutti i nodi** ma, di per se, non garantisce percorsi ottimi **tra i nodi stessi**. 
- a **maglia** anche detti **flat** o **peer to peer**. In questo caso  ogni nodo si calcola **il proprio** albero di instradamento per raggiungere gli altri nodi e, la base station, in particolare. Gli alberi, avendo radice, di volta in volta, diversa sono generalmnte diversi. Il protocollo di instradamento è spesso distribuito e dinamico e rispetto all'altra soluzione garantisce **percorsi ottimali** per **tutti i nodi**, **non solo** per la base station.

![cluster-flat](cluser-flat.png)
