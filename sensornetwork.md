...
- a **stella gerarchica** i nodi dei sensori sono divisi in diversi **cluster**. In ogni cluster, il nodo con il livello di energia **più alto** è generalmente scelto come la testa del cluster o CH. I nodi non-CH comunicano con i CH attraverso il protocollo TDMA, in base alla pianificazione creata dal CH (inviata a tutti i nodi del cluster). Il ruolo del CH viene ruotato allo scopo di distribuire il carico energetico tra i sensori (altrimenti si scaricherebbero per primi). In questa categoria, molto popolare è il protocollo Low Energy Adaptive Clustering Hierarchy (LEACH). I **dispositivi sensori** hanno adesso a disposizione un **percorso ottimizzato** (più breve o più veloce) lungo i rami dell'albero per comunicare con la base station (il gateway). Il **sink tree** o albero di instradamento **ottimo** è **unico** per tutti i cluster. La topologia ad **albero unico** avente per **radice la base station**  (gateway) garantisce un percorso ottimo tra **il gateway e tutti i nodi** ma, di per se, non garantisce la scelta di percorsi ottimi **tra i nodi stessi**. 
- a **maglia** anche detti **flat** o **peer to peer**. I nodi dei sensori hanno caratteristiche simili nella raccolta dei dati, funzionalità, trasmissione e consumo di energia. In questo caso  **ogni nodo** si calcola **il proprio** albero di instradamento ottimo per raggiungere gli altri nodi e la base station. Gli alberi, avendo, di volta in volta, per **radice un nodo** differente sono generalmente anch'essi diversi. Il protocollo di instradamento è spesso distribuito e dinamico e, rispetto all'altra soluzione, garantisce **percorsi ottimali** per **tutti i nodi**, **non solo** per la base station.
