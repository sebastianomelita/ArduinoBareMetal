...
Un'**alternativa** al **ricampionamento nel loop(**) per compensare i **ritardi** di un task, potrebbe essere sfruttare la proprietà di **prerilascio** forzato di un task che possiedono gli interrupt. Se i tick sono **interrupt based** allora essi accadono sempre e comunque nel tempo esatto a loro deputato anche in presenza di un task che ritarda la sua esecuzione. Questo perchè un tick che occorresse ripetutamente su uno stesso task molto lento causa comunque la sua **interruzione** e l'esecuzione di una **ISR** che, pur **non** potendo servire un **nuovo task**, può far partire un algoritmo che **compensi il conteggio** dei tick dei task successivi in maniera da farli accadere al **tempo giusto**. 

Grazie all'espediente del **riordinamento** per tempi decrescenti dei task si può evitare l'interferenza tra task di tick successivi. Anche se task di tick successivi non si interferiscono, rimane sempre l'influenza che un task lento può avere sui task del suo stesso tick che non può essere evitata senza ricampionamento tra un task e l'altro.
