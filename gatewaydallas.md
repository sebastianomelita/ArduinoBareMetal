...
Il **gateway** proposto di seguito esegue lo **scan** degli indirizzi OneWire dei dispositivi presenti sul BUS e li memorizza in un array di array durante il **setup()**. Nel **loop()**, ad ogni iterazione schedulata, **legge** i valori di temperatura per ogni indirizzo e li **impacchetta in un JSON** che manda **via client MQTT** all'applicazione remota. **NSENSORS** è la costante che imposta il numero di sensori presenti sul BUS.
