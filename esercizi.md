...
- **start**. Avvia o riavvia il timer quando si avvera la **condizione di attivazione** posta in punto qualsiasi del loop. Il suo scopo è anche quello di memorizzare il **tempo iniziale** di riferimento a partire dal quale la ```get()``` misurerà il **tempo trascorso** (o elapsed). Chiamate successive alla funzione ```start()``` non modificano questo **tempo iniziale** perchè questo viene marcato come già misurato da un **flag singleton** che controlla che la chiamata alla sottostante funzione ```millis()``` non avvenga più di **una volta**. 
