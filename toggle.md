...
Le attese sono tutte **non bloccanti** e realizzate tramite un timer HW che adopera esso stesso gli **interrupt** per richiamare la funzione di servizio (callback) da eseguire allo scadere del **timeout** del timer. Il timer, utilizzando gli interrupt, è in grado di intervenire in tempo in **tutte le situazioni**, eventualmente anche interrompendo l'esecuzione del codice che impegna intensamente il loop(). Si tratta sicuramente di una realizzazione molto utile avendo la massima efficacia possibile in tutte le situazioni.
