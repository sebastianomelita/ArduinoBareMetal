...
Pur utilizzando gli interrupt, l'efficacia del codice precedente in termini di velocità e responsività è limitata dalla **componente nel loop()** del meccanismo che è sensibile a ritardi di esecuzione che in esso possono essere introdotti da eventuali istruzioni delay() o da blocchi di istruzioni troppo lente posti al suo interno. L'efficacia di questa realizzazione è quindi vanificata dai limiti intrinseci della **schedulazione periodica** effettuata nel loop().
...
Le attese sono tutte non bloccanti e realizzate tramite un timer HW che adopera esso stesso gli **interrupt** per richiamare la funzione di servizio (callback) da eseguire allo scadere del **timeout** del timer.
