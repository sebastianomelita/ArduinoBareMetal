...
Questa tecnica ha lo svantaggio presentare una interferenza temporale dell'esecuzione dei vari task che sono separarti da un ritardo ```delay()```. I task devono essere eseguiti in sequenza e inoltre l'esecuzione del task successivo non può cominciare se prima non termina quella del task corrente. Di fatto un task può rimanere bloccato e quindi non essere responsivo da perte dell'utente, o può non elaborare l'uscita richiesta per un certo tempo.
