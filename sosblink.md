...
- un'altra soluzione potrebbe essere **programmare** il solo thread del **loop() principale** utilizzando una schedulazione basata sui **time tick**. Questa soluzione, nel contesto di queta categoria di problemi (**task contemporanei**), appare **vantaggiosa** perchè:
     ...
     - Inoltre l'efficienza è molto alta sia perchè da un lato non rimane bloccato nessun task in attesa di un altro, sia dall'altro, perchè non sono impegnate schedulazioni sottostanti da parte di un OS come nel caso dei thread, che comunque introducono un certo grado di spreco di risorse (overhead del cambio di contesto). Rimane il problema della maggiore complessità della gestione della **comunicazione** tra le varie **fasi** di uno stesso algoritmo che non è più sequenziale lungo il codice ma **a salti** secondo un **ordine** stabilito da etichette:
