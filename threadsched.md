...
- **"non preemptive" cioè cooperativa**. Se un task di un thread possiede una risorsa (ad esempio la CPU):
     ...
     - sta eseguendo delle istruzioni, allora **il task** non viene interrotto fino al suo **completamento** (run to completition). Per evitare che task lunghi monopolizzino la CPU, si può fare in modo di inframezzare le righe di codice con chiamate yeld(), in modo da causare un periodico rilascio della CPU allo schedulatore.
