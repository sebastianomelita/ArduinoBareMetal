...
- **"cohoperative" cioè cooperativa**. Se un task di un thread possiede una risorsa (ad esempio la CPU) ed è bloccato in attesa di un input o di un una qualsiasi altra risorsa, esso stesso, con un comando inserito **nel codice del task** (ad es. yeld()), decide **spontaneamente** di cedere il controllo della CPU allo **schedulatore** che assegna la risorsa CPU ad **un altro thread** tra quelli che, in quel momento, aspettano di andare in esecuzione (**stato ready**).
