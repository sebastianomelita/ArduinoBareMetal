...
- **prempitive o competitiva**. Se un thread possiede una risorsa (ad esempio la CPU) ed è bloccato in attesa di un input o di un un delay(), viene marcato come **interrompibile**.  Allo scadere di un timer HW, viene interrotto da un segnale di **interrupt** che blocca il flusso di esecuzione corrente (task) e assegna la risorsa CPU ad **un altro thread** tra quelli che, in quel momento, aspettavano di andare in esecuzione (**stato ready**).
- **"cohoperative" cioè cooperativa**. Se un thread possiede una risorsa (ad esempio la CPU) ed è bloccato in attesa di un input o di un un delay(), esso stesso, con un comando inserito nel codice dell'applicazione (ad es. yeld()), rilascia spontaneamente la risorsa cedendo il controllo della CPU allo schedulatore che assegna la risorsa CPU ad **un altro thread** tra quelli che, in quel momento, aspettavano di andare in esecuzione (**stato ready**).
