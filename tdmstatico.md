...
La potenziale **contesa del canale**, da parte di quelle sorgenti che tentano di accedervi **simultaneamente**, è risolta nel dominio del tempo **comprimendo** la durata della trasmissione dei messaggi (rispetto a quello della loro ricezione) in finestre temporali **separate** e di **ugual durata** dette **slot**. Ogni slot è **assegnato** ad una certa sorgente in una posizione fissa all'interno dell'intervallo di tempo che intercorre tra l'arrivo di due **messaggi consecutivi** (lo stesso per tutte le sorgenti). 
