...
Una volta finito il caricamento, gli N messaggi vengono inviati, in **momenti diversi** (TDM), nell’unica porta di uscita e con **velocità maggiore** di quella di arrivo in modo che la trasmissione di tutti avvenga sempre nel tempo T di arrivo di uno solo. La potenziale **contesa per l'accesso** simultaneo al canale da parte delle varie sorgenti è risolta nel **dominio del tempo** comprimendo la trasmissione del messaggio in finestre temporali prestabilite per ciascuna sorgente (**slot**).
