...
La potenziale **contesa del canale**, da parte di quelle sorgenti che tentano di accedervi **simultaneamente**, è risolta nel dominio del tempo **comprimendo** la durata della trasmissione dei messaggi (rispetto a quello della loro ricezione) in finestre temporali di **ugual durata** dette **slot**. Ogni slot è **assegnato** ad una certa sorgente in una posizione **separata** da quella di tutte le altre che è **regolare**, cioè  fissa all'interno dell'intervallo di tempo che intercorre tra l'arrivo di due **messaggi consecutivi** (lo stesso per tutte le sorgenti). 

Il **tempo T**, uguale al **tempo di arrivo** completo di un messaggio di una generica sorgente (**tempo di ricezione** uguale per tutte), viene detto **tempo di trama** o **periodo**. E' quindi suddiviso in **slot** numerati di ugual durata sui quali è **allocata** la trasmissione delle sorgenti, in successione, uno slot dopo l'altro:
