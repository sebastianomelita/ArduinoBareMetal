...
- A **Livello 2** della pila ISO/OSI su trame di **livello pari**, cioè su trame **L2 MAC** (EAPoL). EAP, in questo caso, viene utilizzato per trasportare le **credenziali** (username e password) nella **tratta iniziale**, quella che va dal dispositivo utente ad un **dispositivo di accesso** (NAS, inteso stavolta come Network Access Server) alla rete WiFi, che è, **prima** dell'autenticazione, ancora **priva di indirizzo IP**.
- A **Livello 7** della pila ISO/OSI su trame di livello 7 **RADIUS**. Radius adesso serve a trasportare le **credenziali utente** nella **tratta successiva**, quella che va dall’autenticatore (l’AP) al server RADIUS, dove entrambi i dispositivi sono adesso **dotati di indirizzo IP**, essendo connessi alla rete LAN.

Sul server RADIUS le **credenziali EAP** verranno verificate, **cercando l’username** in un database centrale (di tipo LDAP o MySQL) e, se la verifica avviene con successo, un pacchetto EAP di **autorizzazione** verrà inviato dal server RADIUS verso l'autenticatore NAS che, similmenta ad un ponte levatoio, attiverà l'**accesso dati**, permettendo, per prima cosa, l'acquisizione automatica da parte del dispositivo client dell'**indirizzo IP** via DHCP.
