...
Lo standard IEEE 802.1X è un protocollo di controllo dell'**accesso alla rete LAN basato sulla porta** (Port-Based Network Access Control): impedisce a un dispositivo di comunicare sulla LAN o sulla WLAN finché l'utente non si è autenticato. In pratica la "porta" (fisica su uno switch, o logica/associazione su un access point) resta chiusa a qualunque traffico tranne quello di autenticazione, e si apre solo se le credenziali sono valide.
...
Sul server RADIUS le **credenziali EAP** verranno verificate, **cercando l’username** in un database centrale (di tipo LDAP o MySQL) e, se la verifica avviene con successo, un pacchetto EAP di **autorizzazione** verrà inviato dal server RADIUS verso l'autenticatore NAS che, similmente ad un ponte levatoio, attiverà l'**accesso dati** permettendo, per prima cosa, l'acquisizione automatica da parte del dispositivo client dell'**indirizzo IP** via DHCP.
...
## **Autenticazione di porta (su switch o su AP)**
...
<img src="img/porteradius.png" alt="alt text" width="600">
...
[Dettaglio Captive Portal](approfondimenti/captive_portal.md)
