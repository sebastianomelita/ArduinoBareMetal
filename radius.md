...
- A **Livello 7** della pila ISO/OSI su trame di livello 7 **RADIUS**. Radius adesso serve a trasportare le **credenziali utente** nella **tratta successiva**, quella che va dall’autenticatore (l’AP) al server RADIUS, dove entrambi i dispositivi sono adesso **dotati di indirizzo IP** essendo connessi alla rete LAN. Sul server RADIUS le **credenziali EAP** verranno verificate, **cercando l’username** in un database centrale (di tipo LDAP o MySQL), e se la verifica avviene con successo, un pacchetto EAP di autorizzazione verrà inviato dal server RADIUS verso l'autenticatore NAS che, come un ponte levatoio, attiverà l'accesso dati, permettendo, per prima cosa, l'acquisizione automatica da parte del dispositivo client dell'indirizzo IP via DHCP.
