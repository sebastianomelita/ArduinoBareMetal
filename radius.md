...
Sul server RADIUS le **credenziali EAP** verranno verificate, **cercando l’username** in un database centrale (di tipo LDAP o MySQL) e, se la verifica avviene con successo, un pacchetto EAP di **autorizzazione** verrà inviato dal server RADIUS verso l'autenticatore NAS che, come un ponte levatoio, attiverà l'accesso dati, permettendo, per prima cosa, l'acquisizione automatica da parte del dispositivo client dell'indirizzo IP via DHCP.
