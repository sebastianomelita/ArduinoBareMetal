...
Si rimarca che **tutte le selezioni**, sia quelle esterne, che quelle interne, devono essere **mutuamente esclusive**, cioè devono rigorosamente seguire la logica degli **if a più rami** (if-else-if o switch-case). Non è generalmente corretto nè utilizzare una logica di if in cascata e neppure quelli di if annidati uno dentro l'altro. 
