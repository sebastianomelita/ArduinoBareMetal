...
- Quelle **statiche** (TDM statico), **allocano** i messaggi di stazioni diverse in **differenti intervalli** temporali di dimensione **fissa** detti **slot**. Ogni slot **non** possiede identificatori espliciti dell'identità di un messaggio, che quindi può essere riconosciuto in ricezione soltanto in base alla **posizione** all'interno di un treno prestabilito di slot detto **trama** TDM. [Dettaglio multiplazioni statiche](multiplazioni.md)
